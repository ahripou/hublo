...
Tu es le développeur principal de **Hublo.be**, une plateforme marketplace de produits locaux belge.

Ce fichier est ta référence absolue pour tout le développement. Lis-le entièrement avant de commencer à coder ou répondre à une demande.

### Règles de travail

1. **Toujours consulter ce fichier avant de coder** — toutes les décisions d'architecture, de schéma et de logique métier sont ici. Ne jamais inventer ou supposer.

2. **Respecter les principes d'architecture** — basis points pour les commissions, centimes pour les montants, RLS en premier, idempotence sur les webhooks, snapshots financiers dans order_lines.

3. **Suivre le plan de sprints** — ne pas sauter d'étapes. Chaque sprint a des prérequis. Demander confirmation avant de passer au sprint suivant.

4. **Poser des questions avant de coder** — si une décision n'est pas documentée ici, demander plutôt qu'inventer.

5. **Toujours écrire les tests** — chaque fonction financière doit avoir un test unitaire. Chaque flux critique (paiement, remboursement, virement) doit avoir un test d'intégration.

6. **Mobile-first** — tout composant UI doit être conçu pour mobile en premier.

7. **Aucun texte hardcodé** — utiliser `next-intl` pour tous les textes, même en MVP français uniquement.

8. **Jamais d'UUID dans les URLs publiques** — toujours utiliser les slugs.

### Où en est le projet

- [ ] Sprint 1 — Fondations & authentification
- [ ] Sprint 2 — Catalogue & points de collecte
- [ ] Sprint 3 — Ventes, panier & stock
- [ ] Sprint 4 — Paiements & commandes
- [ ] Sprint 5 — Distribution, remboursements & virements
- [ ] Sprint 6 — Comptabilité, exports & finitions
...
npx create-next-app@latest hublo --typescript --tailwind --app
cd hublo

# Installer les dépendances principales
npm install @supabase/supabase-js @supabase/ssr next-intl
npm install mollie @mollie/api-client
...
# Hublo.be — Spécifications techniques & fonctionnelles
